...
L'énoncé du problème "Collecte des déchets, des bateaux nettoyeurs" :
...
Initialement, un bateau apparait sur chaque base navale.
Chaque bateau va alors récupérer des déchets (tant qu'il n'a pas atteint sa limite de deux déchets en stock),
les déposer dans une base navale (pas forcément celle dont il est issu), et ainsi de suite, pendant mille jours !
...
ramasser les déchets sur sa position, s'il ne transporte pas déjà deux déchets et s'il est seul sur sa position,
se débarrasser des déchets qu'il transporte s'il se trouve sur une base navale,
s'il est sur une base navale ou à la même position qu'un autre navire, partir dans sa direction favorite, sinon il part dans la direction correspondant à sa position.
...
Voici un exemple de solution possible pour une région 10x10